            ...
            <tbody>
                @foreach ($orders as $index => $orderItem)
                    <tr>
                        ...
                        <td>{{ $orderItem->vendor_tmmin ?? '-' }}</td>
                        <td>{{ $orderItem->vendor_akti ?? '-' }}</td>
                        <td>{{ $orderItem->keterangan ?? '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: center; vertical-align: bottom;">
                        @if($order->is_acknowledged && $sarprasUser && $sarprasUser->ttd)
                            <img src="{{ storage_path('app/public/' . $sarprasUser->ttd) }}" style="max-height: 38px;">
                        @else
