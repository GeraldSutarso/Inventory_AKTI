                ...
                <tbody>
                    ...
                    @foreach ($products as $product)
                        ...
                        <td class="p-2 flex gap-2">
                            <button onclick="event.preventDefault(); document.getElementById('generate-qr-{{ $product->id }}').submit();" class="bg-blue-500 py-1 px-4 rounded text-white">
                                Generate QR
                            </button>
                        
                            <form id="generate-qr-{{ $product->id }}" action="{{ route('products.qr', $product->id) }}" method="GET" class="hidden">
                            </form>
                        
                            @if ($product->qr_code)
                                <img src="{{ asset('storage/' . $product->qr_code) }}" class="w-20 h-20"/>
                            @endif
